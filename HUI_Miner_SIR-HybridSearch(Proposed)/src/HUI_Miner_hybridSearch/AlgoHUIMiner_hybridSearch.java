package HUI_Miner_hybridSearch;
 ...
 */
public class AlgoHUIMiner_hybridSearch {
	...
	* the idea is to always reuse the same buffer to reduce memory usage. */
     ...
     * Default constructor
     */
    public AlgoHUIMiner_hybridSearch() {
    }
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (Pair pair : revisedTransaction) {
                    ...
                    //mantain minimum iutil and rutil value
...
//        int compare = mapItemToTWU.get(item1) - mapItemToTWU.get(item2);
    ...
    private void huiMiner(int[] prefix, int prefixLength, UtilityList pUL, List<UtilityList> ULs, int minUtility)
        ...
        for (int i = 0; i < ULs.size(); i++) {
            ...
            //display Utiliy List of Itemset 
            // System.out.println("\n\n");
            // System.out.println("ItemName="+X.item+"\nsumofIutil="+X.sumIutils+"\nsumofRutil="+X.sumRutils+"\nMinIutil="+X.minIutils+"\nMinRutil="+X.minRutils);
            ...
            if (X.sumIutils + X.sumRutils >= minUtility) {
                ...
                for (int j = i + 1; j < ULs.size(); j++) {
                    ...
                    //Before construction take the decision as extension will construct or not 
//                    int exputility = 0;
//                    UtilityList k;
//                    if (mapItemToSupport.get(X.item) >= mapItemToSupport.get(Y.item)) {
//                        k = X;
//                        exputility = (int) X.sumIutils + (int) Y.sumIutils + (int) Y.sumRutils - ((abs(mapItemToSupport.get(X.item) - mapItemToSupport.get(Y.item))) * ((int) k.minIutils));
//                    } else {
//                        k = Y;
//
//                        exputility = (int) X.sumIutils + (int) Y.sumIutils + (int) Y.sumRutils - ((abs(mapItemToSupport.get(X.item) - mapItemToSupport.get(Y.item))) * ((int) k.minIutils + (int) k.minRutils));
//                    }

                    // we construct the extension pXY 
                    // and add it to the list of extensions of pX
                    // if(exputility>=minUtility)
                    // {
    ...
    int binarySearch(UtilityList py, int find, int low, int high) {
        ...
        while (low <= high) {
            ...
            if (py.elements.get(mid).tid == find) {
                return mid;
            } else if (py.elements.get(mid).tid < find) {
                low = mid + 1;
            } else {
                high = mid - 1;
            }
...
//        System.out.println("Number of comparison=" + while_loop);
