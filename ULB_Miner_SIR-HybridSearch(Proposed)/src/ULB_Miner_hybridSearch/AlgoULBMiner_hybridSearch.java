package ULB_Miner_hybridSearch;
...
import hui_miner.UtilityList;
import tools.MemoryLogger;

/**
 * This is an implementation of the "ULB-Miner" algorithm for High-Utility
 * Itemsets Mining as described in the conference paper : <br/><br/>
 *
 ...
 */
public class AlgoULBMiner_hybridSearch {
     ...
     * Default constructor
     */
    public AlgoULBMiner_hybridSearch() {
...
//		// if the same, use the lexical order otherwise use the TWU
        ...
        int compare = (int) (mapItemToSup.get(item1) - mapItemToSup.get(item2));
    ...
    int binarySearch(int find, int low, int high) {
        ...
        while (low <= high) {
            ...
            if (utilityListBuffer.getIthElementInCurrentUtilityList(mid).tid == find) {
                return mid;
            } else if (utilityListBuffer.getIthElementInCurrentUtilityList(mid).tid < find) {
                low = mid + 1;
            } else {
                high = mid - 1;
            }
    ...
    private boolean construct(int PPosition, int pXPosition, int pYPosition, int minUtility, int endPosition, int itemY, int totalUtility) {
        ...
        //System.out.println("X-> "+countX+"Y-> "+countY+" | SIR->"+SIR);
        ...
        while (posX < countX && posY < countY) {
            ...
            if (ex.tid == ey.tid) {
                ...
                utilityListBuffer.addElementToCurrentUtilityList(ex.tid, ex.iutils + ey.iutils - epIutil,
                        ey.rutils);
                posX++;
                posY = (posY + SIR <= countY - 1) ? posY + SIR : countY - 1;

            } else if (ex.tid > ey.tid && posY != countY - 1) {

                posY = (posY + SIR <= countY - 1) ? posY + SIR : countY - 1;
                Updated_BS_Start = posY - SIR + 1;

            } else if (ex.tid < ey.tid && SIR != 1) {

                
                utilityListBuffer.selectCurrentUtilityList(pYPosition);
                BS_Find_Index = binarySearch(ex.tid, Updated_BS_Start, posY - 1);

                if (BS_Find_Index != -1) {

                    ey = utilityListBuffer.getIthElementInCurrentUtilityList(BS_Find_Index);
